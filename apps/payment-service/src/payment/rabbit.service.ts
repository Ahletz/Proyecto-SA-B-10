...
@Injectable() export class RabbitService implements OnModuleDestroy{
 private readonly logger=new Logger(RabbitService.name); private conn:ChannelModel|null=null; private channel:Channel|null=null; private shuttingDown=false;
 constructor(private readonly c:ConfigService){}
 private async ch(){if(this.channel)return this.channel;this.conn=await amqp.connect(this.c.getOrThrow<string>('RABBITMQ_URL'));this.channel=await this.conn.createChannel();this.exitOnUnexpectedClose(this.conn,this.channel);for(const e of ['bank.events','bank.events.retry','bank.events.dlx'])await this.channel.assertExchange(e,'topic',{durable:true});return this.channel;}
 ...
 // Sin reconexión propia: si RabbitMQ cierra la conexión o el canal (p. ej. reinicio del broker), el servicio
 // quedaría vivo sin consumir ni publicar. Se termina el proceso para que Kubernetes/Docker lo levante de nuevo.
